...
describe("ChatView MCP app messaging", () => {
  ...
  it("projects a dispatching queued turn into the transcript immediately", () => {
    const payload = {
      persona: { kind: "none" as const },
      text: "Instant hello",
      sendOptions: { displayText: "Instant hello" },
    };
    const record = {
    ...
    const timelineProps = mocks.messageTimelineSpy.mock.calls.at(-1)?.[0] as {
      messages: Array<{
        id: string;
        metadata?: { queueRecordId?: string };
      }>;
      footer?: ReactNode;
    };
    expect(timelineProps.messages).toHaveLength(1);
    expect(timelineProps.messages[0]).toMatchObject({
      id: "queued:record-1",
      metadata: { queueRecordId: "record-1" },
    });
    expect(timelineProps.footer).not.toBeNull();
    expect(screen.getByText("Instant hello")).toBeVisible();
    ...
    expect(inputProps.composerActions.canSteerQueuedMessage).toBe(false);
    expect(inputProps.composerActions.onSteerQueuedMessage).toBeUndefined();
    expect(inputProps.composerActions.queuedMessage).toBeNull();
    expect(inputProps.composerActions.queuedMessages).toEqual([]);
  });

  it("replaces the optimistic queued turn with its committed message without duplication", () => {
    const payload = {
      persona: { kind: "none" as const },
      text: "Only once",
    };
    ...
    mocks.useChatSessionController.mockReturnValue({
      ...
      messages: [
        {
          id: "committed-user-message",
          role: "user",
          created: Date.now(),
          content: [{ type: "text", text: "Only once" }],
          metadata: { userVisible: true, queueRecordId: "record-2" },
        },
      ],
      queue: {
        queuedMessage: payload,
    ...
    expect(timelineProps.messages).toEqual([
      expect.objectContaining({ id: "committed-user-message" }),
    ]);
  });
