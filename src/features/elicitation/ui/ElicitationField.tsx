...
import { elicitationFieldKind } from "@/features/elicitation/lib/elicitationFieldKind";
...
function MultiSelectField({
  ...
  const customOtherSelected =
    companionName != null && typeof otherValue === "string";
    ...
    >
      ...
      <div className="grid gap-2">
        {visibleOptions.map((option, index) => {
          ...
          const atLimit =
            maxItems != null &&
            visibleSelected.length + (otherSelected ? 1 : 0) >= maxItems;
            ...
            >
              <Checkbox
                ...
                disabled={
                  !otherSelected &&
                  maxItems != null &&
                  visibleSelected.length >= maxItems
                }
...
function UnsupportedField({
  schema,
  fieldId,
}: ElicitationFieldProps & { fieldId: string }) {
  const { t } = useTranslation("chat");
  const raw = schema as Record<string, unknown>;
...
export function ElicitationField(props: ElicitationFieldProps) {
  const fieldId = useId();
  switch (elicitationFieldKind(props.schema)) {
    case "multi-select":
    ...
    default:
      // Preserved in the request and the response, but never dressed up as a
      // control that would misdescribe what the agent asked for.
      return <UnsupportedField {...props} fieldId={fieldId} />;
  }
}
